  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed left-1/2 top-[15%] z-50 w-full max-w-[560px] -translate-x-1/2 rounded-[6px] border border-[--color-borde] bg-white shadow-[0_12px_32px_rgba(0,0,0,0.10)]"
          aria-describedby={undefined}
        >
          <Dialog.Title className="sr-only">Búsqueda global</Dialog.Title>

          <div className="flex items-center gap-2 border-b border-[--color-borde] px-3">
            <Search className="size-4 text-[--color-texto-suave]" aria-hidden />
            <input
              autoFocus
              value={consulta}
              onChange={(evento) => {
              ...
              className="h-11 flex-1 bg-transparent text-[14px] outline-none placeholder:text-[--color-texto-suave]"
            />
          </div>

          <div className="max-h-[320px] overflow-auto p-1.5">
            {resultados.length === 0 ? (
              <p className="px-2.5 py-6 text-center text-[13px] text-[--color-texto-suave]">
                    ...
                    .map((resultado) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {resultado.detalle && (
                            <span className="cifra shrink-0 text-[11px] text-[--color-texto-suave]">
                              {resultado.detalle}
                            </span>
                          )}
                        </button>
          ...
          <div className="flex items-center gap-3 border-t border-[--color-borde] px-3 py-2 text-[11px] text-[--color-texto-suave]">
